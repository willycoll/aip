...
- The collection where the resource is being added **should** map to the URI
  path.
  - The collection's parent resource identifiers **should** be called
    `{resource}_id`, and **should** be the only variables in the URI path.
...
message CreateBookRequest {
  // The publisher resource where this book will be created.
...
- All parent resource identity fields **must** be included unless the resource
  being created is a top-level resource. They **should** be called `{resource}_id`.
...
- The `id` field on the resource **must** be ignored.
