...
- The collection whose resources are being listed **should** map to the URI
  path.
  - The collection's parent resource identity (`publisher_id` in the above example)
    **should** be the only variables in the URI path. All remaining parameters
...
message ListBooksRequest {
  ...
  // A cursor, received from a previous `ListBooks` call.
...
- All parent resource identity fields **must** be included unless the resource being listed is a
  top-level resource.
  - The fields **should** be [annotated as required][aip-203].
  - The fields **should** identify the [resource type][aip-123] of the resource
    being listed.
- The `limit` and `cursor` fields, which support pagination, **must**
  be specified on all list request messages. For more information, see
  [AIP-158][].
  - The comment above the `limit` field **should** document the maximum
...
- The `cursor` field **must** be included on all list request messages.
...
```proto
message ListBooksData {
  repeated Book books = 1;
}

message ListBooksMeta {
  // The number of results returned in ListBooksData
  int32 count = 1;

  // A token, which can be sent as `cursor` to retrieve the next page.
  // If this field is omitted, there are no subsequent pages.
  string next_cursor = 2;
}

message ListBooksResponse {
  ListBooksData data = 1;
  ListBooksMeta meta = 2;
}
```

- The response message **must** include a `data` field with a repeated field
  corresponding to the resources being returned, and **should not** include
  any other repeated fields unless described in another AIP (for example, AIP-217).
...
- The `meta.next_cursor` field, which supports pagination, **must** be included
  on all list response messages. It **must** be set if there are subsequent pages,
  and **must not** be set if the response represents the final page. For more
  information, see AIP-158.
- The `meta` message **may** include a `int32 count` (or `int64 count`)
  field with the number of items in the collection.
  - The value **may** be an estimate (the field **should** clearly document
    this if so).
  - If filtering is used, the `count` field **should** reflect the size of
...
message **should** contain a field for each filter. Filtering is described in
