...
resource's URI (for example, `/v1/publishers/{publisher_id}/books/{book_id}`) in
...
- The response message **must** be the resource itself. (There is no
  ...
  - The response **should** include the fully-populated resource, and **must**
    include any fields that were sent and included in the request body unless
...
- The resource's identity fields **should** map to the URI path.
  - The `{resource}_id` fields **should** be the only variable in the URI
    path.
...
message UpdateBookRequest {
  // The book to update.
  //
  // The book's `{resource}_id` fields are used to identify the book to update.
...
- The request message **must** contain a field for the resource.
  ...
  - A `{resource}_id` fields **must** be included in the resource message.
    They **should** be called `{resource}_id`.
- A field mask **should** be included in order to support partial update. It
  **must** be of type `google.protobuf.FieldMask`, and it **should** be called
  ...
  - The REST API **may** infer the `update_mask` based on the field sent in the
    request body.
...
message UpdateBookRequest {
  // The book to update.
  //
  // The book's `{resource}_id` fields are used to identify the book to be updated.
  Book book = 1 [(google.api.field_behavior) = REQUIRED];
